...
(function () {
    ...
    const MARK_ALL_READ_URL = "{{ route('admin.notifications.markAllRead') }}";
    ...
    function render(data) {
        ...
        data.notifications.forEach((n) => {
            ...
            item.href = n.url;
            item.className = 'block px-4 py-3 hover:bg-slate-50 flex gap-3 ' + (n.is_read ? '' : 'bg-blue-50/50');
            item.innerHTML = `
    ...
    function fetchNotifications() {
        return fetch(NOTIF_URL, { headers: { 'Accept': 'application/json' } })
            .then((res) => res.json())
    ...
    // Polling tiap 20 detik buat update badge count (tanpa mark-as-read)
    fetchNotifications().then((data) => data && render(data));
    setInterval(() => fetchNotifications().then((data) => data && render(data)), 20000);
})();
</script>

@else

{{-- Role lain (Dosen/Kajur) masih bell statis sampai notif-nya digarap --}}
<button
    class="relative w-11 h-11 rounded-xl bg-slate-100 hover:bg-slate-200 transition flex items-center justify-center">
    ...
    <span
        class="absolute -top-1 -right-1 bg-red-500 text-white text-[10px] w-5 h-5 rounded-full flex items-center justify-center">
        0
    </span>
