...
@section('page-title','Edit Master Device')
...
<div class="bg-white rounded-lg shadow p-6">
    ...
    <form action="{{ route('dosen.devices.update',$device) }}" method="POST">
        @csrf
        @method('PUT')
        ...
        <div class="mb-5">
            ...
            <select name="node" class="w-full border rounded-lg px-4 py-2" required>
                ...
                @for($i=1;$i<=10;$i++)
                    <option value="{{ $i }}" {{ $device->node == $i ? 'selected' : '' }}>
                        Node {{ $i }}
                    </option>
                @endfor
            </select>
        ...
        <div class="mb-5">
            ...
            <input type="text" name="nama_device" value="{{ $device->nama_device }}"
        ...
        <div class="mb-5">
            ...
            <textarea name="keterangan" rows="3"
                      class="w-full border rounded-lg px-4 py-2">{{ $device->keterangan }}</textarea>
        </div>
        ...
        <div class="mb-6">
            ...
            <select id="jumlahSensor" class="w-full border rounded-lg px-4 py-2">
                ...
                @for($i=1;$i<=5;$i++)
                    <option value="{{ $i }}" {{ $device->sensors->count() == $i ? 'selected' : '' }}>
                        {{ $i }} Sensor
                    </option>
                @endfor
            </select>
...
const existingSensors = @json($existingSensors);

function buatAktuatorHTML(sensorIndex, data = null) {
    const nama = data ? data.nama_aktuator : '';
    const on = data ? data.kondisi_on : '';
    const off = data ? data.kondisi_off : '';
...
function tambahAktuator(sensorIndex, data = null) {
    const container = document.getElementById('aktuatorContainer_' + sensorIndex);
    if (!container) return;
    container.insertAdjacentHTML('beforeend', buatAktuatorHTML(sensorIndex, data));
}
...
function renderSensor(total) {
    ...
    for (let i = 0; i < total; i++) {
        const data = existingSensors[i] || null;

        const namaSensor = data ? data.nama_sensor : '';
        const parameter = data ? data.parameter : '';
        ...
        <div class="border rounded-lg p-5 bg-gray-50">
            ...
            <div class="mt-4">
                ...
                <textarea
                    ...
                    rows="2"
                    class="w-full border rounded-lg px-3 py-2">${keterangan}</textarea>
            </div>
    ...
    for (let i = 0; i < total; i++) {
        ...
        if (data && data.actuators && data.actuators.length > 0) {
            data.actuators.forEach(function (akt) {
                tambahAktuator(i, akt);
            });
        } else {
            tambahAktuator(i);
        }
...
renderSensor(existingSensors.length);
